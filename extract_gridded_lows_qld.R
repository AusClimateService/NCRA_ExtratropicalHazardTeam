...
rcm2=c(rep("CCAM",6),rep("BARPA",7),rep("CCAM-UQ-DES",15))
subdir=c(rep("/CCAM-CMIP6/",6),rep("/BARPA/BARPA-R/",7),rep("/CCAM-QLD/",15))

projS=c(rep("proj100_lows_rad2cv0.5_hq89",6),rep("proj100_lows_rad2cv0.5_ia39",6),"proj100_lows_rad2cv0.5_py18",rep("proj100_lows_rad2cv0.5",15))
projU=paste0("500hPa_z/",c(rep("proj100_lows_rad2cv1_hq89",6),rep("proj100_lows_rad2cv1_ia39",6),"proj100_lows_rad2cv1_py18",rep("proj100_lows_rad2cv1",15)))

ssp="historical"
#year1=c(rep(1951,6),rep(1960,7))
year1=rep(1960,length(model))
...
for(i in c(14:20,22:length(model))) # Have to skip EC-Earth3 due to 500hPa data issue
{
  print(i)
  ...
  for(y in 1:length(years))
  {
    ...
    ##Upper
     if(agency[i]=="UQ-DES") udir=paste0(bdir,subdir[i],model[i],"_",rcm[i],"/",member[i],"/",ssp,"/",projU[i]) else  udir=paste0(bdir,subdir[i],model[i],"/",ssp,"/",projU[i])
      ...
      ##Surface
     if(agency[i]=="UQ-DES") sdir=paste0(bdir,subdir[i],model[i],"_",rcm[i],"/",member[i],"/",ssp,"/",projS[i]) else  sdir=paste0(bdir,subdir[i],model[i],"/",ssp,"/",projS[i])
