...
for year in {2015..2099}; do
 cdo timmax $indir/pr_AGCD-05i_${fname}*${year}${yend}.nc $odir/tmp_RX1D_${year}.nc

 if [[ $year -eq 2015 ]]; then
  cp $indir/pr_AGCD-05i_${fname}*${year}${yend}.nc $odir/tmp1.nc
 else
  cdo mergetime $indir/pr_AGCD-05i_${fname}*${year}${yend}.nc $indir/pr_AGCD-05i_${fname}*$((year-1))${yend}.nc $odir/tmp1.nc
 fi
