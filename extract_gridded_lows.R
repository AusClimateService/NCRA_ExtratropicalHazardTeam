...
rcm=c(rep("CCAM",6),rep("BARPA",7))
model=c("ACCESS-CM2","ACCESS-ESM1-5","CESM2","CMCC-ESM2","CNRM-ESM2-1","EC-Earth3",
        "ACCESS-CM2","EC-Earth3","NorESM2-MM","ACCESS-ESM1-5","CMCC-ESM2","CESM2","MPI-ESM1-2-HR")

projS=c(rep("proj100_lows_rad2cv0.5_hq89",6),rep("proj100_lows_rad2cv0.5_ia39",6),"proj100_lows_rad2cv0.5_py18")
projU=paste0("500hPa_z/",c(rep("proj100_lows_rad2cv1_hq89",6),rep("proj100_lows_rad2cv1_ia39",6),"proj100_lows_rad2cv1_py18"))
...
for(i in 1:length(model))
{
  ...
  for(y in 1:length(years))
  {
    ...
    ##Upper
    if(rcm[i]=="BARPA") udir=paste0(bdir,"/BARPA/BARPA-R/",model[i],"/",ssp,"/",projU[i]) else
      if(rcm[i]=="CCAM") udir=paste0(bdir,"/CCAM-CMIP6/",model[i],"/",ssp,"/",projU[i]) 
      ...
      ##Surface
      if(rcm[i]=="BARPA") sdir=paste0(bdir,"/BARPA/BARPA-R/",model[i],"/",ssp,"/",projS[i]) else
        if(rcm[i]=="CCAM") sdir=paste0(bdir,"/CCAM-CMIP6/",model[i],"/",ssp,"/",projS[i]) 
  ...
  ncout <- nc_create(paste0(odir,"/",rcm[i],"/lows_",rcm[i],"_",model[i],"_",ssp,".nc"),cyc_def) #force_v4=T)
